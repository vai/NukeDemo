...
namespace NameGenerator {
  internal static class Program {
    ...
    static Random Rand { get; } = new Random(DateTime.UtcNow.Subtract(DateTime.Today.Date).Seconds);

    static void Main(string[] args) {

      int number;
      if (args.Length > 0) {
        int.TryParse(args[0], NumberStyles.Any, CultureInfo.InvariantCulture, out number);
      }
      else {
        number = 5;
      }
    ...
    static void WriteName() {
      ...
      var scientist = RandomItem(Scientists);
      var name = $"{modifier}-{scientist}";
